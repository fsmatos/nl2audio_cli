...
from .validation import check_openai_api_key, ValidationError
from .logging import get_logger, log_info, log_debug, log_warning, log_error
...
TTS_PRICING = {
    "gpt-4o-mini-tts": 0.00015  # $0.00015 per 1K characters
}

def estimate_tts(text: str, voice: str = "alloy", model: str = "gpt-4o-mini-tts") -> Dict[str, Any]:
    """
    ...
    logger.info(f"TTS estimation: {total_chars} chars, {num_chunks} chunks, "
...
def _find_safe_break_point(text: str, max_chars: int, start_pos: int = 0) -> int:
    ...
    for i in range(min(max_chars, len(text) - 1), start_pos, -1):
        if _is_sentence_end(text[i]):
            ...
            if i > 0 and text[i-1].isupper() and i < len(text) - 1 and text[i+1].isspace():
                continue
            return i + 1
...
def _optimize_chunk_size(chunks: List[str], target_chars: int = 3500) -> List[str]:
    ...
    for chunk in chunks:
        ...
        if (len(current_chunk) < target_chars * 0.7 and 
            len(current_chunk) + len(chunk) + 1 <= target_chars):
            if current_chunk:
                current_chunk += " " + chunk
            else:
                current_chunk = chunk
...
def synthesize(text: str, voice: str, out_path: Path, bitrate: str = "64k", max_minutes: int = 60, dry_run: bool = False) -> bytes:
    logger = get_logger()
    ...
    for i, chunk in enumerate(chunks):
        try:
            ...
            if total_ms / 1000 / 60 > max_minutes:
                log_warning(f"Audio length limit exceeded: {total_ms/1000/60:.1f} minutes > {max_minutes} minutes")
    ...
    logger.info(f"All chunks processed successfully. Total audio: {total_ms/1000:.1f} seconds")
