...
def trim_silence(seg: AudioSegment, threshold: float = -35.0, padding_ms: int = 150) -> AudioSegment:
    start_end = silence.detect_nonsilent(seg, min_silence_len=200, silence_thresh=threshold)
    if not start_end:
        return seg
