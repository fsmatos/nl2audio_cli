...
def check_environment() -> None:
    ...
    if missing_vars:
        ...
        error_text.append("\nPlease set these variables in your environment or .env file.", style="white")
        
        console.print(Panel(error_text, title="Environment Validation Failed", border_style="red"))
        raise ValidationError(f"Missing environment variables: {', '.join(missing_vars)}")
...
def check_openai_api_key() -> str:
    ...
    if not api_key.startswith("sk-"):
        raise ValidationError("OPENAI_API_KEY appears to be invalid (should start with 'sk-')")
