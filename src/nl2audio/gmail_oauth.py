...
def authenticate_gmail() -> Tuple[str, Credentials]:
    ...
    try:
        ...
        try:
            keyring.set_password(KEYRING_SERVICE, KEYRING_KEY_FORMAT.format(email=email), creds.to_json())
...
def get_stored_credentials(email: str) -> Optional[Credentials]:
    ...
    """
    try:
        stored_creds = keyring.get_password(KEYRING_SERVICE, KEYRING_KEY_FORMAT.format(email=email))
        if stored_creds:
            creds = Credentials.from_authorized_user_info(json.loads(stored_creds), SCOPES)
            if creds.valid:
                return creds
                ...
                try:
                    ...
                    keyring.delete_password(KEYRING_SERVICE, KEYRING_KEY_FORMAT.format(email=email))
        return None
    except Exception as e:
