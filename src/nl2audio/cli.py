...
def _init_logging(cfg: AppConfig) -> None:
    ...
    setup_logging(log_file=log_file, level=log_level, enable_rich=True)
...
def doctor(
    probe_openai: bool = typer.Option(
        False, "--probe-openai", help="Test OpenAI API connectivity"
    ),
    probe_gmail: bool = typer.Option(
        False, "--probe-gmail", help="Test Gmail connectivity"
    ),
):
    ...
    try:
        ...
        results = validate_runtime(
            cfg, check_openai=probe_openai, check_gmail=probe_gmail
        )
        ...
        for result in results:
            ...
            # Add to table
            table.add_row(
                result.name, status_text, result.message, result.remediation or "N/A"
            )
        ...
        if summary["failed"] > 0:
            ...
            if summary["warnings"] > 0:
                summary_text.append(
                    f", ⚠️ {summary['warnings']} warnings", style="yellow"
                )
            ...
            summary_text.append(
                f"✅ All {summary['passed']} checks passed!", style="green"
            )
            exit_code = 0
...
def add(
    ...
    try:
        ...
        try:
            duration_sec = int(
                len(AudioSegment.from_file(mp3_path, format="mp3")) / 1000
            )
        ...
        console.print(
            Panel.fit(f"Added episode: [bold]{ep_title}[/bold]\nFile: {mp3_path}")
        )
...
def gen_feed():
    ...
    try:
        ...
        with DB(_db_path(cfg)) as db:
            ...
            xml_path = build_feed(
                cfg.output_dir, cfg.feed_title, cfg.site_url, episodes
            )
...
def serve(
    port: int = typer.Option(
        8080, help="Port to serve feed & episodes", min=1, max=65535
    )
):
    ...
    try:
        ...
        with socketserver.TCPServer(("0.0.0.0", port), handler) as httpd:
            ...
            console.print(
                Panel.fit(f"Serving {cfg.output_dir} at http://127.0.0.1:{port}")
            )
            try:
                httpd.serve_forever()
        ...
        with DB(_db_path(cfg)) as db:
            for i, msg in enumerate(messages, 1):
                try:
                    ...
                    db.add_episode(
                        ep_title, msg.source, mp3_path, duration_sec, content_bytes
                    )
                    log_success(f"Email processed successfully: {ep_title}")
                    console.print(
                        f"[green]Added episode from email:[/green] {ep_title}"
                    )

                except Exception as e:
                    log_error(f"Failed to process email '{msg.title}': {e}")
                    console.print(
                        f"[red]Failed to process email '{msg.title}':[/red] {e}"
                    )
                    continue
...
def gmail_test():
    ...
    try:
        ...
        if cfg.gmail.method != "oauth":
            ...
            console.print(
                "[red]Gmail OAuth is not configured. Run 'nl2audio connect-gmail' first.[/red]"
            )
            raise typer.Exit(1)
        ...
        if not creds:
            ...
            console.print(
                "[red]No valid OAuth credentials found. Run 'nl2audio connect-gmail' first.[/red]"
            )
            raise typer.Exit(1)
        ...
        if not label_id:
            ...
            try:
                ...
                console.print(
                    f"📁 Available labels: {[label['name'] for label in labels[:10]]}"
                )
                if len(labels) > 10:
...
def quickstart():
    ...
    try:
        ...
        console.print(
            Panel.fit(
                content, title="📚 nl2audio Quickstart Guide", border_style="blue"
            )
        )
